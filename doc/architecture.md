...
| ap_ext_loot_claim | Two-way corpse loot ownership: xttltable ledger keyed victim -> killer (actor kills via npc_on_hit `from_death_callback`); xevent-hooks `xr_corpse_detection.near_actor` to reserve actor kills from NPC looters within radius; an `ActorMenu_on_item_before_move` veto (xinventory.is_corpse_take / deny_move) denies taking from a living NPC's claimed kill within the same radius |
